...
for ($i = 0; $i -lt $panels.Count; $i++) {
  $panel = $panels[$i]
  $index = $i + 1
  Write-Host "更新分镜 $index/$($panels.Count)..."
  
  # 更新文案
  $env:TEMP = "C:\Users\Administrator\AppData\Local\Temp"
  node C:\Users\Administrator\.openclaw\workspace\skills\zzdh-control-skill-v3.0-new\zzdh-cli.js update-prompt --panel-index $i --paperwork $panel.text 2>&1 | Out-Null
  
  # 更新图片提示词
  node C:\Users\Administrator\.openclaw\workspace\skills\zzdh-control-skill-v3.0-new\zzdh-cli.js update-prompt --panel-index $i --prompt $panel.imagePrompt 2>&1 | Out-Null
  
  # 更新视频提示词
  node C:\Users\Administrator\.openclaw\workspace\skills\zzdh-control-skill-v3.0-new\zzdh-cli.js update-prompt --panel-index $i --video-prompt $panel.videoPrompt 2>&1 | Out-Null
}
